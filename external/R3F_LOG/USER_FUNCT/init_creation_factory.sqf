 ...
 * 
 * @usage nul = [my_factory] execVM "external\R3F_LOG\USER_FUNCT\init_creation_factory.sqf"; // Unlimited credits, all sides allowed, all categories except black list in config_creation_factory.sqf
 * @usage nul = [my_factory, 10000] execVM "external\R3F_LOG\USER_FUNCT\init_creation_factory.sqf"; // 10000 credits, all sides allowed, all categories except black list in config_creation_factory.sqf
 * @usage nul = [my_factory, -1, independent] execVM "external\R3F_LOG\USER_FUNCT\init_creation_factory.sqf"; // Unlimited credits, allow to GUER side, all categories except black list
 * @usage nul = [my_factory, -1, nil, "MEDIUM"] execVM "external\R3F_LOG\USER_FUNCT\init_creation_factory.sqf"; // All sides allowed, use white list MEDIUM in config_creation_factory.sqf
 * @usage nul = [my_factory, -1, nil, ["Car", "Armored"]] execVM "external\R3F_LOG\USER_FUNCT\init_creation_factory.sqf"; // Allow only categories "Car" and "Armored"
